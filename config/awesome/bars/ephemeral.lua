...
local create_button = function (symbol, color, bg_color, hover_color, text)
...
kbmap = create_button("", beautiful.xcolor2, beautiful.xcolor2.."30", beautiful.xcolor1.."50", "de")
...
awesome.connect_signal("evil::volume", function(vol, muted)
    local t = volume:get_all_children()[1]:get_all_children()[3]
    local k = volume:get_all_children()[1]:get_all_children()[2]
    if muted then
        t.markup = helpers.colorize_text(vol .. "%", volume_muted_color)
        k.markup = helpers.colorize_text(volume_symbol, volume_muted_color)
    else
        t.markup = helpers.colorize_text(vol .. "%", volume_unmuted_color)
        k.markup = helpers.colorize_text(volume_symbol, volume_unmuted_color)
    end
end)
